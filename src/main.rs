...
mod tests {
    ...
    mod debian {
        ...
        fn ls() {
            let config = Config {
                root: "debian_container".to_string(),
                command: vec!["ls".to_string()],
            };
            let jail: process::Child = setup_container(config);

            match jail.wait_with_output() {
                Err(message) => panic!("{}", message),
                Ok(mut output) => assert_eq!(
                    str::from_utf8(output.stderr.drain(..).as_slice()).unwrap(),
                    ""
                ),
            }
        }
        ...
        fn debian_update() {
            let config = Config {
                root: "debian_container".to_string(),
                command: "apt-get update".split(' ').map(|s| s.to_string()).collect(),
            };
